...
const runLabel = () => {
  ...
  return s.runtime.value === 'loading' ? (sql() ? 'Loading the database…' : 'Loading Python…') : 'Running…'
}
</script>
  ...
  <div class="veil">
    <section class="sys quest" role="dialog" aria-modal="true">
      ...
      <div class="body">
        ...
        <label>
          <div class="editor-label">{{ sql() ? 'Your SQL' : 'Your solution' }} (saved locally) <small>Tab / Shift+Tab indent · Ctrl+Enter runs tests</small></div>
          <textarea class="editor" v-model="s.notes.value[s.gate.value.id]" spellcheck="false" autocapitalize="off" autocomplete="off"
            wrap="off" :placeholder="sql() ? '-- write your query here' : '# write your Python here'" @keydown="onKey"></textarea>
        </label>
        <div class="row">
          <template v-if="s.gate.value.tests">
            ...
            <span v-else-if="s.runtime.value === 'loading'" class="dim">{{ sql() ? 'First run downloads the runtime (about 10 MB).' : 'First run downloads the Python runtime (about 10 MB).' }}</span>
