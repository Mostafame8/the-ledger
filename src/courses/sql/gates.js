...
{ name:'Arc III — The totals', sub:'The fence stops asking which and starts asking how much.', gates:[
  ...
  g('heavyhitters','C',120,'shape','The heavy hitters','SUM and HAVING',
    ...
    'Pile the transfers up by the account they left, add each pile, and sieve on that total rather than on any single transfer. The sieve that runs after the piles are built is not the one that runs before.'),
...
{ name:'Arc IV — Inside the question', sub:'Some answers need a second question, or a second list, before the first one means anything.', gates:[
  g('abovemean','C',120,'filter','Above the average','Scalar subquery',
    ...
    'A SELECT that gives back exactly one row and one column can stand anywhere a plain number can, brackets and all. Work the average out inside the same statement and the answer is still right after the next transfer lands.'),
  g('neverswiped','B',160,'filter','Never swiped in','NOT EXISTS',
    ...
    'Ask the badge log a yes-or-no question about the staff row you are standing on, and keep the people it answers no to. What the inner SELECT returns never matters, only whether it finds anything at all.'),
