...
{ name:'Arc V — The long con', sub:'Stay in character. The mark changes; the play does not.', gates:[
  g('readtheroom','A',200,'behaviour','Reading the room','State',
    ['The mark at the bar has three moods. Calm, he makes small talk. Wary, he gives short answers. Alarmed, he calls it in. The play has one script, and the script has to read the mood.',
     'Dax\'s Mark has a mood string and an if-chain in every method. Add a fourth mood and six methods change.',
     '“Make each mood an object,” Marguerite says. “The mood knows how the mark talks and which mood comes next. The mark holds the current mood and asks it. No ifs in the mark.”'],
    'Write state classes Calm, Wary and Alarmed, each with a name attribute (its lowercase class name), talk() returning "small talk", "short answers" and "calls it in", and next() returning the state that follows (Calm to Wary to Alarmed; Alarmed stays Alarmed). Then class Mark that starts Calm, with nudge() moving to the next state, talk() delegating to the current state, and mood giving the current state\'s name. Stretch: Mark holds no if on the mood anywhere.',
    'Mark stores a state object. nudge() replaces it with state.next(). talk() returns state.talk(). mood is state.name. Every decision lives in the state classes.'),
  g('runsheet','A',220,'structure','Same sheet, different night','Template method',
    ['Every job runs the same sheet: check the gear, do the thing, wipe the place down. Only the middle line changes, and Dax has three copies of the whole sheet with three different middles.',
     '“One sheet,” Marguerite says. “The base class owns run() and the order. Each job fills in the middle line. If a job wants a different first line, it fills that too.”',
     'Dax: “And a job that forgets the middle?” “Blows up when run, loudly. Better than a quiet night doing nothing.”'],
    'Write class Job with run() returning the list [self.prepare(), self.execute(), self.cleanup()], where prepare() returns "gear checked", cleanup() returns "wiped down", and execute() raises NotImplementedError. Then class VaultJob(Job) whose execute() returns "vault emptied", and class SafehouseJob(Job) whose execute() returns "files copied" and whose prepare() returns "keys copied". Stretch: add a hook before_execute() that does nothing by default.',
    'run() lives once, in Job, and calls self.prepare, self.execute, self.cleanup. Subclasses override the steps, never run itself.'),
  g('roombyroom','A',240,'behaviour','Room by room','Iterator',
    ['The vault plan is a list of rooms, some locked. Dax wants every question about the plan answered by building the full list of rooms first, then throwing most of it away.',
     '“Walk it,” Marguerite says. “Room by room. A for loop asks the vault for the next room and stops when there is none. Nothing is built that is not needed.”',
     '“__iter__ with a yield inside makes the vault walkable. A generator over that walk keeps only the open rooms. first_locked stops at the first one it finds.”'],
    'Write class Vault built with a list of (room, locked) tuples, stored as vault.rooms, whose __iter__ yields the room names in order. Then open_rooms(vault), a generator that yields only the rooms that are not locked, and first_locked(vault) returning the first locked room name or None, stopping as soon as it finds one and never building a list. Stretch: make the same Vault walkable twice.',
    'Store the tuples. __iter__ loops over them and yields the name; that alone makes for, list and in work. open_rooms and first_locked walk vault.rooms, not a copy.'),
  g('wholerig','S',300,'creation','Build the whole rig','Combining patterns',
    ['The last job of the season. Marguerite puts one sheet of paper on the bench: hire by role, tell everyone when someone joins, plan a route, go.',
     '“That is three jobs from three different nights,” Dax says, reading it twice. “It is one object,” Marguerite says, “and you already know every shape in it.”',
     'The fence, from the door: “Build it once. I have buyers for the next ten years.”'],
    'Write class Crew with register(role, cls) recording a class under a role; hire(role) building and returning an instance of the registered class (KeyError for an unknown role) and emitting "hired" with the new member; on(event, fn) subscribing to an event; emit(event, payload) calling that event\'s subscribers in order and returning how many were called; plan(strategy) storing a route object; and go(city) returning strategy.path(city). Stretch: Crew never names a member class or a route class.',
    'A dict for roles, a dict of lists for events, one slot for the route. hire is the window, emit is the wire, go hands the city to whatever route was planned.'),
]},
];
