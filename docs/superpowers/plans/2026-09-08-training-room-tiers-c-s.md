...
| greedy | A/200 | intervals, heaps | task, jump, timetable, epilogue | `max_events(intervals)` sort by end, take non-overlapping: ask `end` and `count` (5 stops) | `min_coins_greedy(amount, coins)` with canonical coins `[25, 10, 5, 1]` (2 blanks: how many of this coin, remainder) | `max_non_overlapping(intervals)` count of intervals kept |
...
| string-search | S/320 | strings | kmp, palsub | `prefix_table('abab')`: ask the table after each fill (4 stops) | `expand(s, lo, hi)` widen while ends match, return `(lo+1, hi-1)` as a tuple (2 blanks: loop test, the two moves) | `count_occurrences(text, pat)` overlapping occurrences, naive scan allowed |
