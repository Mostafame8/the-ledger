...
- `src/courses/patterns/` — the design-patterns course "The Blueprint": same layout as algorithms (`course.js`, `gates.js`, `index.js`, `tests.js`, `training/` with `tools/`). Stats `structure`, `behaviour`, `creation`. No `scene` anywhere in this course (three.js never loads there). Six tools: class, abstract, dunder, closure, dataclass, generator. Lessons (plan 3 of the courses spec) list the gate ids they prepare. Reference solutions in `scripts/solutions/patterns/`.
- `src/courses/oop/` — the object-oriented Python course "The Manifest": same layout as patterns. Stats `shape`, `kin`, `protocol`. No `scene` anywhere. Six tools: dict, function, tuple, decorator, exception, type. Reference solutions in `scripts/solutions/oop/`. Lessons list the gate ids they prepare.
...
- Per course: lesson ids never collide with that course's gate or tool ids; every tool in a course's armoury is required by at least one of its lessons (`tests/training-tools.test.mjs`). The patterns course sets no `scene`, and its prose never names a pattern or principle (only `algo`, spot options and mandated class or method names may). The oop course sets no `scene` either; its prose may use Python syntax words (`class`, `self`, `super()`, `__eq__`, `@property`) but never the textbook nouns (inheritance, polymorphism, encapsulation, descriptor, metaclass, MRO, protocol, dunder, abstract) outside `algo` and spot steps.
...
- Patterns course ("The Blueprint"): after Halden the crew has money and no kit, and builds it properly this time. Dax welds one lump; the learner reshapes it into parts. Arcs: I The workbench (principles) · II The forge (making things) · III The disguise (wrapping what you cannot change) · IV The play (deciding and reacting) · V The long con (state, the run sheet, walking the vault, the whole rig). Gate titles are scene names; only `algo` names the pattern. Lessons: The one-job rule · Bolt-on, never saw-off · The stand-in · The socket · Parts, not bloodlines · The order window · The rig, piece by piece · One radio · The foreign plug · Layers on the coat · The front desk · Pick the play · The tripwire · The undo button · The mood of the mark · The run sheet · Walk the vault · The whole rig.
